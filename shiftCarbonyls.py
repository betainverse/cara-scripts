...
"""
shiftAllCarbonyls.py takes a CARA repository file and alters the
chemical shift of all carbonyl spins to compensate for an incorrect
sweepwidth used in processing, discovered only after all peaks were picked.
Modify the script to specify your particular shift. Search for newshift

Aliases are also shifted if the flag -a is used. 

Script modified from shiftAllCarbons.py by Robert Mallis.
"""
...
# Convert spins using formula 
# newshift = ((shift - Center_old) * (sweep_new / sweep_old) + center_new))
        ...
        if tag == 'C':
            ...
            for pos in positions:
                ...
                if spec == '0' or alias==True:
                    newshift = str((shift-175.962)*(3393.569/12019.23)+175.962)
